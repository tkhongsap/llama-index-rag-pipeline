...
class FlexibleCSVConverter:
    """Flexible CSV to Document converter that works with different schemas"""
...
def main():
    ...
    input_dir = project_root / "data" / "input_docs"  # Root directory path
    ...
    converter = FlexibleCSVConverter(input_csv, output_dir)
    
    # Setup configuration (auto-generate from CSV analysis)
    config = converter.setup_configuration(config_name="candidate_profiles", auto_generate=True)
