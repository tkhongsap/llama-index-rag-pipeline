"""
Section-based parser for structured Thai land deed documents.
Extends existing DocumentProcessor patterns for section-aware chunking.
"""
...
# Handle imports for existing patterns
try:
...
class LandDeedSectionParser:
    """Section-aware parser for structured Thai land deed documents."""
    ...
    def __init__(
        ...
        Args:
            dataset_config: Configuration for field mappings
        ...
        # Reuse existing DocumentProcessor for metadata extraction
        self.document_processor = DocumentProcessor(dataset_config)
        ...
        # Section patterns matching DocumentProcessor structure
    ...
    def parse_simple_document_to_sections(
        self, 
        simple_doc: SimpleDocument
    ) -> List[TextNode]:
        """
        Parse a SimpleDocument into section-based chunks.
        Maintains compatibility with existing DocumentProcessor output.
        
        Args:
            simple_doc: SimpleDocument from existing DocumentProcessor
        ...
        # Extract base metadata from existing document
        base_metadata = simple_doc.metadata.copy()
        
        # Create key info chunk first (always most important)
        key_info_chunk = self._create_key_info_chunk(simple_doc.text, base_metadata)
        nodes.append(key_info_chunk)
        
        # Parse sections from the structured text
        section_chunks = self._extract_section_chunks(simple_doc.text, base_metadata)
        nodes.extend(section_chunks)
        
        # If no sections found, fall back to sentence splitting
        if len(nodes) == 1:  # Only key info chunk
            logger.warning(f"No sections found in document {base_metadata.get('doc_id', 'unknown')}, using sentence splitting")
            fallback_nodes = self._fallback_to_sentence_splitting(simple_doc.text, base_metadata)
    ...
    def parse_documents_batch(
        self, 
        simple_docs: List[SimpleDocument]
    ) -> List[TextNode]:
        """Parse a batch of documents into section-based chunks."""
        all_nodes = []
        
        for doc in simple_docs:
            try:
                doc_nodes = self.parse_simple_document_to_sections(doc)
                all_nodes.extend(doc_nodes)
                logger.debug(f"Parsed {len(doc_nodes)} chunks from document {doc.metadata.get('doc_id', 'unknown')}")
            except Exception as e:
                logger.error(f"Error parsing document {doc.metadata.get('doc_id', 'unknown')}: {e}")
                continue
        
        logger.info(f"Parsed {len(all_nodes)} total chunks from {len(simple_docs)} documents")
        return all_nodes
            ...
            if match:
                ...
                if len(section_content) > self.chunk_size * 2:
                    ...
                    section_node = TextNode(
                        text=f"## {self._get_section_title(section_name)}\n{section_content}",
    ...
    def _fallback_to_sentence_splitting(
        ...
        """Fallback to standard sentence splitting if no sections found."""
        
        temp_doc = Document(text=document_text)
        fallback_chunks = self.sentence_splitter.get_nodes_from_documents([temp_doc])
